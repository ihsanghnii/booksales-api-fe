...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("userInfo", JSON.stringify(response.user)); // json.stringify buat ubah json jdi string

      return navigate(response.user.role === "admin" ? "/admin" : "/");
    } catch (error) {
      setError(error?.response?.data?.message); // untuk pesan error
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (token && decodedData.success && decodedData.success) {
      navigate("/admin"); 
    }
